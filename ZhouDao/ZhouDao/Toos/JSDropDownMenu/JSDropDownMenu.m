...
#define viewHeight   ([UIScreen mainScreen].bounds.size.height -104.f)*0.8f
...
//向上的标示箭头
@property(nonatomic,strong) UIImageView *tapArrowImageView;
@property(nonatomic,copy)NSString *perforeSelect;
...
- (void)setDataSource:(id<JSDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        CALayer *bgLayer = [self createBgLayerWithColor:TopColor andPosition:bgLayerPosition];
...
//        [tempIndicators addObject:indicator];
        ...
        //箭头
        UIImageView *jiantou = [[UIImageView alloc] initWithFrame:CGRectMake(titlePosition.x + title.bounds.size.width / 2 + 5, self.frame.size.height / 2 - 2, 11, 6)];
        jiantou.tag = i + 100;
        [jiantou setImage:[UIImage imageNamed:@"jiantou"]];
        [self.layer addSublayer:jiantou.layer];
        [tempIndicators addObject:jiantou.layer];
...
- (CATextLayer *)createTextLayerWithNSString:(NSString *)string withColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    CGFloat sizeWidth = (size.width < (self.frame.size.width / _numOfMenu) - LeftAndRightMargins) ? size.width : self.frame.size.width / _numOfMenu - LeftAndRightMargins;
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    /************************此处修改标题颜色 和字体颜色*************************************/
    CATextLayer *textLayer = (CATextLayer *)self.titles[tapIndex];
    NSString *tapIndexStr =[NSString stringWithFormat:@"%ld",(long)tapIndex];
    
    if (![tapIndexStr isEqualToString:_perforeSelect]) {
        [_tapArrowImageView removeFromSuperview];
        _tapArrowImageView =nil;
        
        _tapArrowImageView =[[UIImageView alloc]initWithImage:[UIImage imageNamed:@"sanjiao"]];
        _tapArrowImageView.frame = CGRectMake(textLayer.position.x, textLayer.position.y+15, 45/3, 24/3);
        [self addSubview:_tapArrowImageView];
        
    }else
    {
        if (_tapArrowImageView) {
            [_tapArrowImageView removeFromSuperview];
            _tapArrowImageView =nil;
        }else
        {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        if (i != tapIndex) {
            ...
            CATextLayer *textLayer = (CATextLayer *)self.titles[i];
            textLayer.foregroundColor = [UIColor colorWithRed:83.f/255.0f green:83.f/255.0f blue:83.f/255.0f alpha:1.0f].CGColor;

            [(CALayer *)self.bgLayers[i] setBackgroundColor:TopColor.CGColor];
        }
    }
    ...
    if (displayByCollectionView) {
        ...
        if (tapIndex == _currentSelectedMenudIndex && _show) {
            ...
            [(CALayer *)self.bgLayers[tapIndex] setBackgroundColor:TopColor.CGColor];
        } else {
...
//            textLayer.foregroundColor = [UIColor colorWithRed:83.f/255.0f green:83.f/255.0f blue:83.f/255.0f alpha:1.0f].CGColor;
            ...
            CATextLayer *textLayer = (CATextLayer *)self.titles[tapIndex];
            textLayer.foregroundColor = KNavigationBarColor.CGColor;
...
- (void)backgroundTapped:(UITapGestureRecognizer *)paramSender
{
    ...
    [(CALayer *)self.bgLayers[_currentSelectedMenudIndex] setBackgroundColor:TopColor.CGColor];
}
...
- (void)animateLeftTableView:(UITableView *)leftTableView rightTableView:(UITableView *)rightTableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        if (leftTableView) {
            ...
            leftTableViewHeight = viewHeight;
//            leftTableViewHeight = ([leftTableView numberOfRowsInSection:0] > 5) ? (5 * leftTableView.rowHeight) : ([leftTableView numberOfRowsInSection:0] * leftTableView.rowHeight);
        ...
        if([self.dataSource haveRightTableViewInColumn:_currentSelectedMenudIndex]){
            if (rightTableView) {
                ...
                rightTableViewHeight = viewHeight;
...
//    UILabel *titleLabel = (UILabel*)[cell viewWithTag:1];
    ...
    if (leftOrRight == 1) {
        ...
        if ([titleLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
            titleLabel.textColor = KNavigationBarColor;
...
//            [cell addSubview:accessoryImageView];
            ...
            /**
             *  判断修改 当选中的是全部时候 就把右边的第一个 全部 染色
             */
            NSString *aboveTit = [self.dataSource menu:self titleForRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:0 leftRow:_leftSelectedRow row:_leftSelectedRow]];
            
            if ([aboveTit isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
...
- (void)confiMenuWithSelectRow:(NSInteger)row leftOrRight:(NSInteger)leftOrRight{
    ...
    // 0 左边   1 右边    左边行leftRow   右边行row
    NSString *aboveTit = [self.dataSource menu:self titleForRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow row:row]];
    if ([aboveTit isEqualToString:@"全部"]){
        aboveTit = [self.dataSource menu:self titleForRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:0 leftRow:_leftSelectedRow row:_leftSelectedRow]];
    }
    title.string = aboveTit;
