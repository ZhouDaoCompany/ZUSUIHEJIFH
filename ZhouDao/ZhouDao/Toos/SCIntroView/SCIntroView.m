...
- (void)setUp {
    ...
    if (self.introViewPageControl.hidden == NO) {
        self.introViewPageControl = [[TAPageControl alloc] initWithFrame: CGRectMake(self.frame.size.width*0.1, self.frame.size.height*self.doneButtonLocation +20.f, self.frame.size.width*0.8, 10)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if ((self.scrollView.contentOffset.x / pageWidth) >= self.contentImages.count-1) {
        [self addSubview:self.doneButton];
        self.introViewPageControl.hidden = YES;
    }else{
        [UIView animateWithDuration:0.2f animations:^{
            self.introViewPageControl.hidden = NO;
...
- (UIButton *)doneButton {
    if ([self.dataSource respondsToSelector:@selector(doneButtonInIntroView:)]) {
        ...
        if (CGPointEqualToPoint(doneButton.frame.origin, CGPointZero)) {
            float widthx = kMainScreenHeight - self.frame.size.height*self.doneButtonLocation;
            doneButton.frame = CGRectMake(widthx+10.f, self.frame.size.height*self.doneButtonLocation, kMainScreenWidth - 2*widthx-20.f, 40);
        }
