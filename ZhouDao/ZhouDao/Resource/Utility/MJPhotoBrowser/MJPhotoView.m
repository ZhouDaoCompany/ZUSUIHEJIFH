...
- (void)showImage
{
    if (_photo.firstShow) { // 首次显示
        _imageView.image = [UIImage imageNamed:@"gov_tupian"];//_photo.placeholder; // 占位图片
...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        // 直接显示进度条
//        [_photoLoadingView showLoading]; //change by zmm 隐藏这一行代码
        ...
        [_imageView sd_setImageWithURL:_photo.url
            ...
            if (receivedSize > kMinProgress) {
                if (loading && photoView && expectedSize != 0.0)
                {
                    loading.progress = (float)receivedSize/expectedSize;
                }//加了判断 if (loading && expectedSize != 0.0) change by zmm
            }
...
- (void)dealloc
{
    ...
    // 取消请求
//    [_imageView setImageWithURL:[NSURL URLWithString:@"file:///abc"]]; change by zmm
}
@end