...
+ (ZDURLSessionTask *)_requestWithUrl:(NSString *)url
    ...
    if (absoluteURL == nil) {
        ZDAppLog(@"URLString无效，无法生成URL。可能是URL中有中文，请尝试Encode URL");
        return nil;
    }
...
+ (ZDURLSessionTask *)uploadFileWithUrl:(NSString *)url
    ...
    if ([NSURL URLWithString:uploadingFile] == nil) {
        ZDAppLog(@"uploadingFile无效，无法生成URL。请检查待上传文件是否存在");
        return nil;
    }
    ...
    if (uploadURL == nil) {
        ZDAppLog(@"URLString无效，无法生成URL。可能是URL中有中文或特殊字符，请尝试Encode URL");
        return nil;
    }
...
+ (ZDURLSessionTask *)downloadWithUrl:(NSString *)url
    ...
    session = [manager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (error == nil) {
            ...
            ZDAppLog(@"Download success for url %@",url);
            
        } else {
            [self handleCallbackWithError:error fail:failure];
            
            ZDAppLog(@"Download fail for url %@, reason : %@",url,[error description]);
        }
    }];
...
+ (id)cahceResponseWithURL:(NSString *)url parameters:params {
    ...
    if (url) {
        ...
        if (data) {
            cacheData = data;
            ZDAppLog(@"Read data from cache for url: %@\n", url);
        }
    }
...
+ (void)cacheResponseObject:(id)responseObject request:(NSURLRequest *)request parameters:params {
    if (request && responseObject && ![responseObject isKindOfClass:[NSNull class]]) {
            ...
            [[NSFileManager defaultManager] createDirectoryAtPath:directoryPath
                                      withIntermediateDirectories:YES
                                                       attributes:nil
                                                            error:&error];
            if (error) {
                ZDAppLog(@"create cache dir error: %@\n", error);
                return;
            }
        }
        ...
        if (data && error == nil) {
            ...
            if (isOk) {
                ZDAppLog(@"cache file ok for request: %@\n", absoluteURL);
            } else {
                ZDAppLog(@"cache file error for request: %@\n", absoluteURL);
            }
        }
    }
}
